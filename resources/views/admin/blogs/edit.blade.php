...
<div class="flex flex-col gap-10 px-5 mt-5">
    <div class="breadcrumb flex items-center gap-[30px]">
        ...
        <a href="{{ route('dashboard.blogs.index') }}" class="text-[#7F8190] last:text-[#0A090B] last:font-semibold">Manage Blogs</a>
        <span class="text-[#7F8190] last:text-[#0A090B]">/</span>
        <a href="#" class="text-[#7F8190] last:text-[#0A090B] last:font-semibold ">Edit Blog</a>
    </div>
</div>
<div class="header flex flex-col gap-1 px-5 mt-5">
    <h1 class="font-extrabold text-[30px] leading-[45px]">Edit Blog</h1>
    <p class="text-[#7F8190]">Edit your blog post</p>
</div>

<div class="">
    <form method="POST" action="{{ route('dashboard.blogs.update', $blog->id) }}" enctype="multipart/form-data" class="flex flex-col gap-[30px] w-[800px] mx-[70px] mt-10">
        @csrf
        @method('PUT')
        <div class="flex gap-5 items-center mb-4">
            <input type="file" name="image" id="icon" class="peer hidden" onchange="previewFile()" data-empty="{{ $blog->image ? 'false' : 'true' }}">
            <div class="relative w-[200px] h-[200px] rounded-full overflow-hidden peer-data-[empty=true]:border-[3px] peer-data-[empty=true]:border-dashed peer-data-[empty=true]:border-[#EEEEEE]">
                <div class="relative file-preview z-10 w-full h-full {{ $blog->image ? '' : 'hidden' }}">
                    <img src="{{ $blog->image ? asset('storage/' . $blog->image) : '' }}" class="thumbnail-icon w-full h-full object-cover" alt="thumbnail">
                </div>
        ...
        <div class="flex flex-col gap-[10px]">
            ...
            <div class="flex items-center w-full h-[52px] p-[14px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                ...
                <input type="text" class="font-semibold placeholder:text-[#7F8190] placeholder:font-normal w-full outline-none" placeholder="Judul" name="title" value="{{ $blog->title }}" required>
            </div>
        </div>

        <div class="flex flex-col gap-[10px]">
            <p class="font-semibold">Content</p>
            <textarea id="content-blog" name="content" class="font-semibold placeholder:text-[#7F8190] placeholder:font-normal
            w-full outline-none">{{ $blog->content }}</textarea>
        </div>

        <div class="mt-4">
            <button type="submit" class="h-[52px] p-[14px_20px] bg-[#2B82FE] rounded-full font-bold text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#2B82FE4D] text-center">Update Blog</button>
        </div>
    </form>
</div>
